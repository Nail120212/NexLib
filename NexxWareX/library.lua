...
local Library = { 
    ...
    local Themes = {
        ["Preset"] = {
            ...
            ["Accent 2"] = Color3.fromRGB(40, 55, 75),
            ["Accent 3"] = Color3.fromRGB(190, 230, 255),
            ["Accent 4"] = Color3.fromRGB(200, 235, 255),
            ["Glass"] = Color3.fromRGB(255, 255, 255),
            ["Text"] = Color3.fromRGB(245, 250, 255),
            ["Muted"] = Color3.fromRGB(170, 190, 210),
        }
    }
...
Items["SubTitle"] = Library:Create("TextLabel", {
    ...
    TextXAlignment = Enum.TextXAlignment.Left
                ...
                Library:Create("UICorner", {
                    Name = "\0",
                    Parent = Items["Minimize"].Instance,
                ...
                Items["Close"] = Library:Create("TextButton", {
                    ...
                    BackgroundTransparency = 0.9,
                    Text = "×",
                    TextColor3 = Color3.fromRGB(255, 140, 150),
                    FontFace = Library.Font,
                    TextSize = 18,
                    AutoButtonColor = false,
                    ZIndex = 21,
                ...
                Items["Minimize"]:Connect("MouseButton1Down", function()
            ...
            function Window:SetOpen(Bool)
                ...
                local fab = Items["OpenAndClose"]
                local logo = Items["Logo"]
                if fab and fab.Instance then
                    if Bool then
                        fab:Tween({BackgroundTransparency = 1, Size = UDim2.new(0, 40, 0, 40)}, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.In))
                        if logo then logo:Tween({ImageTransparency = 1}, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.In)) end
                        task.delay(0.12, function()
                            if Window.IsOpen then
                                fab.Instance.Visible = false
                            end
                        end)
                    else
                        fab.Instance.Visible = true
                        fab.Instance.Size = UDim2.new(0, 40, 0, 40)
                        fab.Instance.BackgroundTransparency = 1
                        if logo then logo.Instance.ImageTransparency = 1 end
                        fab:Tween({BackgroundTransparency = 0.15, Size = UDim2.new(0, 52, 0, 52)}, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out))
                        if logo then logo:Tween({ImageTransparency = 0}, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)) end
                    end
                end
            end
            ...
            Library:Connect(Items["SearchInput"].Instance:GetPropertyChangedSignal("Text"), function()
                ...
                local q = string.lower(Items["SearchInput"].Instance.Text)
                local first
                for Index, Value in PageSearchData do 
                    local Name = Value.Name
                    local Element = Value.Item
                    local hit = q == "" or string.find(string.lower(Name), q, 1, true) ~= nil
                    Element.Instance.Visible = hit
                    if hit and q ~= "" and not first then
                        first = Element
                        for _, d in Element.Instance:GetDescendants() do
                            if d:IsA("TextLabel") and string.find(string.lower(d.Text), q, 1, true) then
                                d.TextColor3 = Color3.fromRGB(255, 230, 80)
                                task.delay(1.2, function()
                                    if d and d.Parent then
                                        d.TextColor3 = Color3.fromRGB(199, 199, 212)
                                    end
                                end)
                            end
                        end
