...
local Library do 
    ...
    local Themes = {
        ...
        ["Amethyst"] = {
            ["Background"] = FromRGB(12, 10, 18),
            ["Outline"] = FromRGB(30, 22, 45),
            ["Inline"] = FromRGB(20, 16, 32),
            ["Accent"] = FromRGB(180, 130, 255),
            ["Text"] = FromRGB(235, 228, 255),
            ["Element"] = FromRGB(28, 22, 42)
        },
        ["Rose"] = {
            ["Background"] = FromRGB(14, 10, 12),
            ["Outline"] = FromRGB(38, 20, 26),
            ["Inline"] = FromRGB(24, 14, 18),
            ["Accent"] = FromRGB(255, 105, 150),
            ["Text"] = FromRGB(255, 235, 240),
            ["Element"] = FromRGB(32, 18, 24)
        },
        ["Ember"] = {
            ["Background"] = FromRGB(14, 10, 8),
            ["Outline"] = FromRGB(38, 22, 14),
            ["Inline"] = FromRGB(24, 14, 10),
            ["Accent"] = FromRGB(255, 140, 60),
            ["Text"] = FromRGB(255, 240, 228),
            ["Element"] = FromRGB(32, 20, 14)
        },
        ["Arctic"] = {
            ["Background"] = FromRGB(8, 12, 18),
            ["Outline"] = FromRGB(18, 28, 42),
            ["Inline"] = FromRGB(12, 20, 30),
            ["Accent"] = FromRGB(90, 190, 255),
            ["Text"] = FromRGB(220, 238, 255),
            ["Element"] = FromRGB(16, 26, 38)
        },
        ["Gold"] = {
            ["Background"] = FromRGB(14, 12, 8),
            ["Outline"] = FromRGB(36, 28, 12),
            ["Inline"] = FromRGB(22, 18, 10),
            ["Accent"] = FromRGB(255, 210, 60),
            ["Text"] = FromRGB(255, 248, 220),
            ["Element"] = FromRGB(30, 24, 12)
        },
        ["Lime"] = {
            ["Background"] = FromRGB(8, 14, 8),
            ["Outline"] = FromRGB(18, 34, 16),
            ["Inline"] = FromRGB(12, 22, 10),
            ["Accent"] = FromRGB(140, 255, 80),
            ["Text"] = FromRGB(228, 255, 220),
            ["Element"] = FromRGB(16, 28, 14)
        },
    }
    ...
    Library.ApplyTheme = function(self, ThemeName)
        ...
        if not ThemeData then
            warn("[NexxTo] Theme not found: " .. tostring(ThemeName))
            return
        end
        for Key, Color in ThemeData do
            Library:ChangeTheme(Key, Color)
        end
    end

    Library.SetTransparent = function(self, Bool, Amount)
        local alpha = Amount or 0.45
        local Frame = Library.Holder and Library.Holder.Instance
        ...
        for _, Desc in Frame:GetDescendants() do
            if Desc:IsA("Frame") or Desc:IsA("ScrollingFrame") then
                if Desc.BackgroundTransparency < 1 then
                    Desc.BackgroundTransparency = Bool and alpha or 0
                end
            end
        end
    end

    Library.SetSize = function(self, Width, Height)
        for _, Win in Library.ThemeItems do
            if Win.Item and Win.Item.ClassName == "Frame" and Win.Item.Name == "\0" then
                local inst = Win.Item
                if inst.Size.X.Offset >= 500 and inst.Size.Y.Offset >= 400 then
                    inst.Size = UDim2.new(0, Width or 673, 0, Height or 511)
                    break
                end
            end
        end
...
RunService.Heartbeat:Connect(function()
    ...
    Countdown.Text = string.format("%02dh %02dm %02ds", hours, minutes, seconds)
            ...
            function Window:SetSize(Width, Height)
                Width = Width or 673
                Height = Height or 511
                Items["MainFrame"].Instance.Size = UDim2New(0, Width, 0, Height)
            end

            function Window:SetTransparent(Bool, Amount)
                local alpha = Amount or 0.45
                local MainInst = Items["MainFrame"].Instance
                if not MainInst then return end
                for _, Desc in MainInst:GetDescendants() do
                    if (Desc:IsA("Frame") or Desc:IsA("ScrollingFrame")) and Desc.BackgroundTransparency < 0.99 then
                        Desc.BackgroundTransparency = Bool and alpha or 0
                    end
                end
                MainInst.BackgroundTransparency = Bool and alpha or 0
            end
...
Library.CreateWindow = function(self, Data)
    ...
    if Data.Size then
        ...
        if type(Data.Size) == "table" then
            SizeX, SizeY = Data.Size[1], Data.Size[2]
        end
        Window:SetSize(SizeX or Data.SizeX, SizeY or Data.SizeY)
    end

    if Data.Transparent ~= nil then
        Window:SetTransparent(Data.Transparent, Data.TransparentAmount)
    end
