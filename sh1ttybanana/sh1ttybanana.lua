...
function Library:NewWindow(ConfigWindow)
    ...
    local ReorderBtn, ReorderIcon = MakeBottomBtn("Reorder", "list-ordered", 1, function(btn, icon)
        ...
        if ReorderMode then
            ...
            for _, t in ipairs(TabElements) do
                ...
                if t.Frame and t.Frame:FindFirstChild("DragHandle") then
                    t.Frame.DragHandle.Visible = true
                end
            end
        else
            ...
            for _, t in ipairs(TabElements) do
                ...
                if t.Frame and t.Frame:FindFirstChild("DragHandle") then
                    t.Frame.DragHandle.Visible = false
                end
            end
        end
    end)
    ...
    MakeBottomBtn("AI", "bot", 2, function()
        if TeddyUI_Premium:FindFirstChild("AIPanel") then
            TeddyUI_Premium.AIPanel:Destroy()
            return
        end

        local groqapi = ""
        local groqprompt = "You are a helpful assistant inside a Roblox UI. You can suggest actions using [TAB:TabName] syntax to navigate tabs. Keep responses concise. Use **bold** for important terms."

        local Panel = Instance.new("Frame")
        Panel.Name = "AIPanel"
        Panel.Parent = TeddyUI_Premium
        ...
        PanelStroke.Color = Color3.fromRGB(80, 0, 160)
        ...
        HeaderFill.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
        HeaderFill.BorderSizePixel = 0
        HeaderFill.Position = UDim2.new(0, 0, 0.5, 0)
        HeaderFill.Size = UDim2.new(1, 0, 0.5, 0)
        HeaderFill.ZIndex = 61

        local AIIcon = Instance.new("ImageLabel")
        AIIcon.Parent = Header
        AIIcon.BackgroundTransparency = 1
        AIIcon.Position = UDim2.new(0, 12, 0.5, -8)
        AIIcon.Size = UDim2.new(0, 16, 0, 16)
        AIIcon.ZIndex = 62
        Library:SetIcon(AIIcon, "bot", Library.Theme.Accent)

        local AITitle = Instance.new("TextLabel")
        AITitle.Parent = Header
        AITitle.BackgroundTransparency = 1
        AITitle.Position = UDim2.new(0, 34, 0, 0)
        AITitle.Size = UDim2.new(1, -70, 1, 0)
        AITitle.Font = Enum.Font.GothamBold
        AITitle.Text = "Groq AI"
        AITitle.TextColor3 = Library.Theme.Text
        AITitle.TextSize = 13
        AITitle.TextXAlignment = Enum.TextXAlignment.Left
        AITitle.ZIndex = 62

        local ModelTag = Instance.new("TextLabel")
        ModelTag.Parent = Header
        ModelTag.BackgroundColor3 = Color3.fromRGB(60, 0, 120)
        ModelTag.BackgroundTransparency = 0
        ModelTag.BorderSizePixel = 0
        ModelTag.Position = UDim2.new(1, -66, 0.5, -9)
        ModelTag.Size = UDim2.new(0, 56, 0, 18)
        ModelTag.Font = Enum.Font.GothamBold
        ModelTag.Text = "llama3"
        ModelTag.TextColor3 = Color3.fromRGB(200, 140, 255)
        ModelTag.TextSize = 10
        ModelTag.ZIndex = 62
        local MTC = Instance.new("UICorner")
        MTC.CornerRadius = UDim.new(1, 0)
        MTC.Parent = ModelTag

        local CloseAI = Instance.new("TextButton")
        CloseAI.Parent = Panel
        CloseAI.BackgroundTransparency = 1
        CloseAI.Position = UDim2.new(1, -28, 0, 10)
        CloseAI.Size = UDim2.new(0, 20, 0, 20)
        CloseAI.Text = "✕"
        CloseAI.TextColor3 = Library.Theme.TextDisabled
        CloseAI.Font = Enum.Font.GothamBold
        CloseAI.TextSize = 12
        CloseAI.ZIndex = 63
        CloseAI.MouseButton1Click:Connect(function() Panel:Destroy() end)
        ...
        InputRow.BackgroundColor3 = Color3.fromRGB(22, 22, 26)
        InputRow.BorderSizePixel = 0
        InputRow.Position = UDim2.new(0, 8, 1, -46)
        InputRow.Size = UDim2.new(1, -16, 0, 36)
        InputRow.ZIndex = 62
        local IRC = Instance.new("UICorner")
        IRC.CornerRadius = UDim.new(0, 8)
        IRC.Parent = InputRow
        local IRSt = Instance.new("UIStroke")
        IRSt.Color = Library.Theme.Stroke
        IRSt.Transparency = 0.5
        IRSt.Parent = InputRow
        ...
        SendBtn.Position = UDim2.new(1, -34, 0.5, -11)
        SendBtn.Size = UDim2.new(0, 24, 0, 22)
        SendBtn.Text = "↑"
        ...
        local function ParseAndRenderText(parent, text, zidx, tabCallback)
...
")
            for li, line in ipairs(lines) do
                ...
                local pos = 1
                while pos <= #line do
                    local bold_s, bold_e, bold_content = line:find("%*%*(.-)%*%*", pos)
                    local tab_s, tab_e, tab_name = line:find("%[TAB:([^%]]+)%]", pos)
                    local next_s = nil
                    local next_type = nil
                    if bold_s and (not tab_s or bold_s < tab_s) then
                        next_s, next_e, next_content, next_type = bold_s, bold_e, bold_content, "bold"
                    elseif tab_s then
                        next_s, next_e, next_content, next_type = tab_s, tab_e, tab_name, "tab"
                    end
                    if next_s then
                        if next_s > pos then
                            table.insert(chunks, {t="text", v=line:sub(pos, next_s-1)})
                        end
                        table.insert(chunks, {t=next_type, v=next_content})
                        pos = next_e + 1
                    else
                        table.insert(chunks, {t="text", v=line:sub(pos)})
                        break
                    end
                end
                ...
                lineFrame.Size = UDim2.new(1, 0, 0, 18)
                lineFrame.AutomaticSize = Enum.AutomaticSize.Y
                lineFrame.ZIndex = zidx

                local xOffset = 0
                for _, chunk in ipairs(chunks) do
                    if chunk.t == "text" or chunk.t == "bold" then
                        local lbl = Instance.new("TextLabel")
                        lbl.Parent = lineFrame
                        lbl.BackgroundTransparency = 1
                        lbl.Position = UDim2.new(0, xOffset, 0, 0)
                        lbl.Size = UDim2.new(1, -xOffset, 0, 0)
                        lbl.AutomaticSize = Enum.AutomaticSize.XY
                        lbl.Font = chunk.t == "bold" and Enum.Font.GothamBold or Enum.Font.Gotham
                        lbl.Text = chunk.v
                        lbl.TextColor3 = chunk.t == "bold" and Color3.fromRGB(220, 160, 255) or Library.Theme.Text
                        lbl.TextSize = 11
                        lbl.TextXAlignment = Enum.TextXAlignment.Left
                        lbl.TextWrapped = true
                        lbl.ZIndex = zidx
                        xOffset = xOffset + lbl.TextBounds.X + 2
                    elseif chunk.t == "tab" then
                        local btn = Instance.new("TextButton")
                        btn.Parent = lineFrame
                        btn.BackgroundColor3 = Color3.fromRGB(50, 0, 100)
                        btn.BorderSizePixel = 0
                        btn.Position = UDim2.new(0, xOffset, 0, 1)
                        btn.Size = UDim2.new(0, 0, 0, 16)
                        ...
                        btn.MouseButton1Click:Connect(function()
                            if tabCallback then tabCallback(chunk.v) end
                        end)
                        xOffset = xOffset + btn.TextBounds.X + 14
                    end
                end
            end
        end
        ...
        local function AddBubble(text, isUser)
            ...
            bubble.BackgroundColor3 = isUser and Color3.fromRGB(60, 0, 120) or Color3.fromRGB(26, 26, 30)
            ...
            if isUser then
                ...
                innerFrame.Parent = bubble
                innerFrame.BackgroundTransparency = 1
                innerFrame.Size = UDim2.new(1, 0, 0, 0)
                innerFrame.AutomaticSize = Enum.AutomaticSize.Y
                innerFrame.ZIndex = 63
                local IL = Instance.new("UIListLayout")
                IL.Parent = innerFrame
                IL.SortOrder = Enum.SortOrder.LayoutOrder
                IL.Padding = UDim.new(0, 2)
                ParseAndRenderText(innerFrame, text, 63, function(tabName)
                    for _, te in ipairs(TabElements) do
                        if te.Title and te.Title:lower() == tabName:lower() then
                            if te.Open then te.Open() end
                            break
                        end
                    end
                end)
        ...
        local function CallGroq(userMsg)
            ...
            local messages = {}
            if groqprompt and groqprompt ~= "" then
                table.insert(messages, {role="system", content=groqprompt})
            end
        ...
        local function Send()
            ...
            InputBox.Text = ""
            AddBubble(txt, true)
            local loadingBubble = AddBubble("...", false)
            task.spawn(function()
                local reply = CallGroq(txt)
        ...
        AddBubble("Hi! I'm your Groq AI assistant. Ask me anything, or I can navigate tabs for you using [TAB:TabName] links.", false)
    end)

    MakeBottomBtn("PlayerCard", "contact", 3, function()
        if TeddyUI_Premium:FindFirstChild("PlayerCardPanel") then
            TeddyUI_Premium.PlayerCardPanel:Destroy()
            return
        end
        ...
        local friendCount = 0
        pcall(function() friendCount = #lp:GetFriendsOnline(200) end)

        local Card = Instance.new("Frame")
        Card.Name = "PlayerCardPanel"
        Card.Parent = TeddyUI_Premium
        Card.BackgroundColor3 = Color3.fromRGB(12, 12, 14)
        Card.BorderSizePixel = 0
        Card.Position = UDim2.new(0, 210, 1, -360)
        Card.Size = UDim2.new(0, 240, 0, 340)
        Card.ZIndex = 60
        Card.ClipsDescendants = false

        local CardC = Instance.new("UICorner")
        CardC.CornerRadius = UDim.new(0, 14)
        CardC.Parent = Card

        local CardSt = Instance.new("UIStroke")
        CardSt.Color = Color3.fromRGB(100, 0, 180)
        CardSt.Transparency = 0.4
        CardSt.Thickness = 1
        CardSt.Parent = Card
        ...
        Banner.ZIndex = 61
        local BanC = Instance.new("UICorner")
        BanC.CornerRadius = UDim.new(0, 14)
        BanC.Parent = Banner
        local BanFill = Instance.new("Frame")
        BanFill.Parent = Banner
        BanFill.BackgroundColor3 = Library.Theme.Accent
        BanFill.BorderSizePixel = 0
        BanFill.Position = UDim2.new(0, 0, 0.5, 0)
        BanFill.Size = UDim2.new(1, 0, 0.5, 0)
        BanFill.ZIndex = 61
        ...
        AvatarBg.BackgroundColor3 = Color3.fromRGB(12, 12, 14)
        AvatarBg.BorderSizePixel = 0
        AvatarBg.AnchorPoint = Vector2.new(0.5, 0)
        AvatarBg.Position = UDim2.new(0.5, 0, 0, 44)
        AvatarBg.Size = UDim2.new(0, 56, 0, 56)
        AvatarBg.ZIndex = 63
        local AvBgC = Instance.new("UICorner")
        AvBgC.CornerRadius = UDim.new(1, 0)
        AvBgC.Parent = AvatarBg
        local AvBgSt = Instance.new("UIStroke")
        AvBgSt.Color = Library.Theme.Accent
        AvBgSt.Thickness = 2
        AvBgSt.Parent = AvatarBg
        ...
        CloseBtn.ZIndex = 65
        CloseBtn.MouseButton1Click:Connect(function() Card:Destroy() end)

        local InfoFrame = Instance.new("Frame")
        InfoFrame.Parent = Card
        InfoFrame.BackgroundTransparency = 1
        InfoFrame.Position = UDim2.new(0, 12, 0, 104)
        InfoFrame.Size = UDim2.new(1, -24, 0, 0)
        ...
        Username.TextColor3 = Library.Theme.Accent
        ...
        local function MakeStat(icon, label, val, order)
            ...
            IcoLbl.Size = UDim2.new(0, 16, 1, 0)
            IcoLbl.Font = Enum.Font.GothamBold
            IcoLbl.Text = icon
            IcoLbl.TextColor3 = Library.Theme.Accent
            IcoLbl.TextSize = 12
            IcoLbl.ZIndex = 63
            ...
            LblText.Position = UDim2.new(0, 28, 0, 0)
            LblText.Size = UDim2.new(0, 80, 1, 0)
            LblText.Font = Enum.Font.Gotham
            LblText.Text = label
            LblText.TextColor3 = Library.Theme.TextDisabled
            LblText.TextSize = 11
            LblText.TextXAlignment = Enum.TextXAlignment.Left
            LblText.ZIndex = 63

            local ValText = Instance.new("TextLabel")
            ValText.Name = "Val_" .. label
            ValText.Parent = Row
            ValText.BackgroundTransparency = 1
            ValText.Position = UDim2.new(0, 110, 0, 0)
            ValText.Size = UDim2.new(1, -118, 1, 0)
            ValText.Font = Enum.Font.GothamBold
            ValText.Text = tostring(val)
            ValText.TextColor3 = Color3.fromRGB(220, 220, 230)
            ValText.TextSize = 11
        ...
        MakeStat("👤", "User ID", lp.UserId, 4)
        MakeStat("🎮", "Acct Age", accountAge .. "d", 5)
        MakeStat("👥", "Friends", friendCount, 6)
        MakeStat("🏠", "Membership", lp.MembershipType.Name, 7)
        local HwidVal = MakeStat("🔑", "HWID", hwid:sub(1,12).."…", 8)
        HwidVal.Parent.MouseButton1Click = nil
        local CopyHwid = Instance.new("TextButton")
        CopyHwid.Parent = HwidVal.Parent
        CopyHwid.BackgroundTransparency = 1
        CopyHwid.Size = UDim2.new(1, 0, 1, 0)
        CopyHwid.Text = ""
        CopyHwid.ZIndex = 65
        CopyHwid.MouseButton1Click:Connect(function()
            if setclipboard then setclipboard(hwid) end
            HwidVal.Text = "Copied!"
            task.delay(1.5, function() HwidVal.Text = hwid:sub(1,12).."…" end)
        end)

        local TimeStat = MakeStat("⏱", "Session", "0m 0s", 9)
        task.spawn(function()
            while Card.Parent ~= nil do
                local elapsed = os.time() - joinTime
                local m = math.floor(elapsed / 60)
                local s = elapsed % 60
                TimeStat.Text = m .. "m " .. s .. "s"
                task.wait(1)
            end
        end)
        ...
        CopyBtn.Position = UDim2.new(0, 12, 1, -44)
        CopyBtn.Size = UDim2.new(1, -24, 0, 30)
        ...
        CopyC.CornerRadius = UDim.new(0, 8)
        ...
        CopyBtn.MouseButton1Click:Connect(function()
            ...
            CopyBtn.Text = "Copied!"
            task.delay(1.5, function() CopyBtn.Text = "Copy Profile Link" end)
        end)
    end)
    ...
    function Tab:T(nameOrConfig, iconName)
        ...
        DragHandle.Position = UDim2.new(1, -28, 0, 0)
        DragHandle.Size = UDim2.new(0, 28, 1, 0)
        DragHandle.Text = ""
        DragHandle.ZIndex = 6
        ...
        DragHandle.MouseButton1Down:Connect(function()
            ...
            tabDragging = true
            tabDragStartY = UserInputService:GetMouseLocation().Y
        end)
        DragHandle.TouchLongPress:Connect(function(pos)
            if not ReorderMode then return end
            tabDragging = true
            tabDragStartY = pos[1].Y
        end)
        ...
        UserInputService.InputChanged:Connect(function(input)
            ...
            local dy = curY - tabDragStartY
            if math.abs(dy) < 16 then return end
            tabDragStartY = curY
            local tabs = {}
            for _, child in ipairs(ScrollingTab:GetChildren()) do
            ...
            elseif dy > 0 and myIdx < #tabs then
                local nxt = tabs[myIdx + 1]
                local tempOrder = TabDisable.LayoutOrder
                TabDisable.LayoutOrder = nxt.LayoutOrder
                nxt.LayoutOrder = tempOrder
            end
        end)
